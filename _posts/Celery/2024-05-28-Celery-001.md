...
작업을 생성하고 브로커에 전송하는 역할을 합니다. 일반적으로 웹 애플리케이션이나 스크립트가 클라이언트가 됩니다. Celery를 사용하여 작업을 정의하고 실행을 요청합니다.
...
가장 기본적인 REST API를 통해 작업을 처리하는 구성을 만들어 봅시다. `docker-compose`를 사용해서 구성해보겠습니다.
...
Celery 앱을 생성하고 `config_from_object`를 통해서 `config` 파일에서 설정을 읽어와서 업데이트 합니다.
...
<div class="file-name">celeryconfig.py</div>
...
`Celery`와 *'작업 큐(Task Queue)'*를 사용하여 분산처리에 대해 알아보았습니다.
...
Celery를 사용하면 RabbitMQ나 redis를 대신해 다른 Broker나 Backend로 손쉽게 변경 가능합니다. 다른 서비스를 이용하여 Celery를 이용한 분산처리를 사용해 보는 것도 좋을 듯합니다. 😊
