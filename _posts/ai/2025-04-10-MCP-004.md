...
{% include template/alert.html
  type="note"
  about='유사한 <a href="https://smithery.ai/" target="_blank">Smithery.ai</a>라는 MCP 마켓플레이스도 사용해보세요!'
%}
...
`.vscode`{:.path} 폴더에 `mcp.json`파일을 생성하고 아래와 같이 내용을 붙여넣습니다. 필요에 따라서 자신에게 맞는 서버주소와 아이디, 패스워드를 입력합시다.
