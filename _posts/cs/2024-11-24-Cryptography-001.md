...
ref-link:
  - type: youtube
    ...
    title: '생활코딩 : 암호학1 - 4.1. 양방향 암호화 - 비대칭키 (공개키 방식)'
  - type: youtube
    url: "https://youtu.be/itlJrSUNSMw?si=Wn5ADjLjP779FUUo&t=583"
    title: '스파르타 연구소 : "전세계를 해킹 가능" 양자컴퓨터로 ... | 김범준 교수'
---
...
- 기밀성(Confidentiality) : 인가된 사용자만 아는 비밀스러운 정보.
- 무결성(Integrity) : 기존의 정보에서 변경되지 않은 정보, 또는 인가된 방법에 의해서 변경되는 정보.
- 가용성(Availability) : 인가된 사용자가 정보를 원할때 지장없이 이용을 보장.
...
![암호화 복호화](/assets/img/content/cs/Cryptography/001.webp)

`> 암호화와 복호화`{:.img-caption}

'암호화(Encryption)'란 일반적인 메시지를 뜻하는 평문(plaintext)을 암호화 알고리즘을 이용하여 코드화된 메시지로 만들어 이해할 수 없도록 암호문(ciphertext)으로 만드는 것을 의미합니다.
...
![암호의 분류](/assets/img/content/cs/Cryptography/002.webp){:.img-m}
...
![Caesar 암호, Scytale 암호](/assets/img/content/cs/Cryptography/003.webp)
...
![대칭키 암호화](/assets/img/content/cs/Cryptography/006.webp)
...
![키 배송 문제](/assets/img/content/cs/Cryptography/007.webp)
...
이러한 문제점은 암호의 가장 중요한 '기밀성'에 치명적입니다. "키를 공유 해야하는데 키를 보내지도 못한다." 이것을 **'키 배송 문제(key distribution problem)'**{:.orange}라 합니다.
...
즉 두 가지 키 모두 암호화/복호화에 사용 가능하며, **반대쪽 키로만 암호화/복호화가 가능합니다.**{:.orange}

![비대칭키 암호화](/assets/img/content/cs/Cryptography/014.webp){:.img-m}
...
**비대칭키**는 복잡한 연산인 '소인수 분해'를 기반해서 동작합니다. 때문에 **많은 양의 데이터를 암호화 하기에는 적절하지 않습니다.**{:.orange} 반면, 대칭키는 암호화와 복호화 속도가 훨씬 빠릅니다.

> "엥? 그럼 데이터 암호화에 안쓰면 뭐에 쓴다는겨?"

물론 비대칭키만 쓰이는 경우도 많이 있습니다. 데이터 암호화가 목적이 아니라면 충분히 좋은 방법이고 이미 많은 기술에서 쓰이고 있습니다. 대표적인 경우가 '전자서명' 입니다. 데이터를 암호화하는 것이 목적이 아니라 **무결성(변조 여부) 검증이 주 목적**{:.orange} 입니다.
...
이놈도 저놈도 사용하려니 뭔가 부족해 보입니다. 그래서 실제로는 **두 방식을 혼합해서 사용하는 경우가 많습니다.**{:.orange}
...
2. 사용자와 Github는 SSH에서 사용할 동일한 대칭키(세션 키)를 생성 (Diffie-Hellman 키 교환)
3. 사용자는 개인키로 서명을 만들어 GitHub 서버에 전송
4. GitHub 서버는 공개키로 서명을 복호화하여 검증

![비대칭 키](/assets/img/content/cs/Cryptography/015.webp){:.img-m}
...
![대칭키 사용](/assets/img/content/cs/Cryptography/016.webp){:.img-m}
