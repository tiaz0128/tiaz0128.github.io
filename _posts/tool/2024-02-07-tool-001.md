...
**반드시 기억해야 할 점은 위에서 찾은 Host 의 주소와 WSL 의 주소는 언제든 랜덤하게 변경될 수 있는 동적인 ip 주소입니다.**{:.orange}
...
`/etc/wsl.conf` 파일은 WSL 이 실행될때 마다 동작을 설정하는데 사용됩니다. 여기서 [boot] 섹션을 추가하고 `command` 옵션을 작성합니다. 동작은 `.env` 파일에 Host 의 주소를 구해서 변경 할 수 있게 세팅해 줍니다.
