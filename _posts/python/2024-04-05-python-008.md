...
catalogue: "python"
subject: "pattern"
title: "디자인 패턴 - 데코레이터(Decorator)"
description: "데코레이터(Decorator) 패턴에 대해서 알아보겠습니다."
author: tiaz0128
permalink: /python/8
...
`Beverage`는 기본 컴포넌트를 정의하는 추상 클래스입니다. `cost()`와 `recipe()` 메소드를 추상 메소드로 정의하여 모든 음료가 이를 구현하도록 강제합니다.
...
`Coffee` 클래스는 `Beverage`의 구체적인 구현체입니다. `cost()`와 `recipe()` 메소드를 오버라이드하여 구현합니다.
...
`Decorator`를 상속받아 기본 기능을 유지하면서 추가 기능을 더합니다. `cost()`와 `recipe()` 메소드를 오버라이드하여 기존 음료에 모카나 우유를 추가하는 기능을 구현합니다.
