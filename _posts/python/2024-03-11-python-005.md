...
파이썬을 직접 여러 버전을 설치하고 관리하는 것은 매우 불편하고 빡셉니다. 제가 추천하는 방법은 `pyenv`입니다.
...
`pip`는 2008년에 처음 만들어졌습니다. "_Python Package Installer_" 약자로, 파이썬 패키지 관리자로써 표준 라이브러리의 일부로 포함되어 있습니다.
...
설치 방법은 간단합니다. [Poetry 설치 페이지](https://python-poetry.org/docs/#installation)를 보시고 따라하시면 됩니다.

### 1.pipx 설치

`pipx`는 파이썬 애플리케이션을 격리된 가상 환경에서 설치하고 실행할 수 있게 해주는 도구입니다. [pipx 설치를 진행합니다.](https://pipx.pypa.io/stable/)
...
이제 내가 원하는 프로젝트 폴더로 이동해서 `Poetry`를 세팅해줍니다. `pyproject.toml` 파일이 생성됩니다.

- `-n`: 입력 정보 생략
- `--python`: 파이썬 버전 정보 입력. [SemVer(Semantic Versioning)](https://semver.org/lang/ko/) 규칙을 따릅니다.
- `--no-root`: 프로젝트 자체를 설치하지 않고 의존성 관리만을 위해 Poetry를 사용하는 옵션

```bash
$ poetry init -n --python=^3.12 --no-root
```
...
`Poetry` 기본 설정으로 패키지를 설치하게 되면 가상환경이 `Poetry`가 사용하는 기본 폴더에 각 가상환경(.venv)가 생성됩니다. 또는 프로젝트 내에 가상환경을 생성하게 할 수도 있습니다.

아래의 옵션을 통해 `pyproject.toml` 파일이 존재하는 프로젝트 내에서 가상 환경(.venv)이 만들어지게 세팅하는 옵션을 활성화 시킬 수 있습니다.
...
여기까지 좀 더 나은 파이썬 개발 환경을 구축하는 방법에 대해 알아보았습니다! 이것봐 각 상황에 알맞은 관리 도구들이 많이 있으니, 자신에게 맞는 파이썬 개발 환경을 구축하는 방법을 찾아 보시길 바랍니다. 😊
