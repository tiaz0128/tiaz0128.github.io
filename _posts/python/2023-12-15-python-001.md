...
UTC는 컴퓨터 시스템과 네트워킹에서 데이터 기록, 로그 관리, 시간 동기화 등에 중요하게 사용됩니다. 아래와 같이 여러가지 프로그래밍 언어에서 간단한 코드를 통해 UTC 가 사용되고 있음을 확인 할 수 있습니다.
...
지역 시간은 UTC 시간에 해당 지역의 오프셋을 더하거나 빼서 계산합니다. 예를 들어, UTC 시간이 0인 (=그냥 UTC 로 표시 할 수 있습니다.) **영국이 크리스마스 이브 = 2023-12-24 일 16:00 시** 이면, 시간대 오프셋이 `+09:00`인 **한국은 크리스마스 당일 2023-12-25 일 0시** 입니다.
...
현재 시스템의 로컬 시스템 시간을 반환합니다. 기본 호출(인수 없이)은 **시간대 정보가 없는 naive datetime** 객체를 반환합니다. 즉, `datetime.now()`로 생성된 datetime 객체는 시간대를 알 수 없습니다. `tzinfo` 속성으로 확인 가능합니다. UTC와 같은 구체적인 시간대와 비교하거나 변환하는 것이 명시적으로는 불가능합니다.
...
UTC 을 반환합니다.  그러나 역시나 **시간대 정보가 없는 naive datetime** 객체를 반환합니다. 그리고 **python 3.12 버젼 부터는 deprecated** 되었기 때문에 **사용하지 않는 것을 권장**합니다.
...
print(utc_now)        # 2023-12-24 16:00:00.000000
...
어웨어 객체는 다른 어웨어 객체와의 상대적인 위치를 파악할 수 있습니다. 일광 절약 시간(=DST), 썸머 타임 정보를 적용 가능합니다. 즉, 명확한 시간 사용 및 계산이 가능합니다.
...
print(now)        # 2023-12-24 16:00:00.000000+00:00
...
UTC 는 시간을 국제 표준시이며, 많은 시스템과 프로그래밍에서도 이를 사용하고 활용합니다. UTC 를 정확하게 표시하는 방법은 **ISO 8601** 를 사용합니다. 하지만 위에서 출력된 결과는 어떤가요? 위에서 봤던 **코드에는 함정**이 있습니다. 다시 한번 잘 보시길 바랍니다.
...
# 코드에서 만든 시간
2023-12-24 16:00:00.000000+00:00
2023-12-25 00:00:00.000000+09:00

# ISO 표준
2023-12-24T16:00:00.000000Z
