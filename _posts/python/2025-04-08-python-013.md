...
description: "uv에 대해서 알아보겠습니다. uv는 Rust로 작성된 매우 빠른 Python 패키지 및 프로젝트 관리 도구입니다. "
...
`uv add`, `uv remove`를 통해 원하는 패키지를 추가, 삭제할 수 있습니다.
...
`pyproject.toml`{:.path} 파일이 있는 경우에는 `uv sync`를 사용 할 수 있습니다. 또는 `uv pip install`를 통해 `requirements.txt`{:.path}에서 패키지를 설치할 수 있습니다.
