...
NGINX가 서버 앞에서 먼저 요청을 받아서 서버에 전달하는 대표적인 리버스 프록시입니다. 우리가 사용할 프록시는 클라이언트에서 동작하는 포워드 프록시 입니다.
...
이때까지 실습의 경우는 브라우저에서 웹 프록시를 써서 웹 트래픽만 EC2로 전달했습니다. 아래의 명령으로 8080 포트를 통하지 않고, 테스트 해보면 트래픽이 프록시로 전달되지 않는 것을 확인 할 수 있습니다.
...
Windows에서는 '수동 프록시 서버 설정 변경'으로 검색하면 수동 프록시를 적용 할 수 있습니다.

![수동 프록시](/assets/img/content/AWS/004/008.png){:.img-s}
