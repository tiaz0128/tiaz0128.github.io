...
[소스코드 Link](https://github.com/tiaz0128/docker-react){:target="_blank"}
...
4. `환경`{:.path} - `추가 구성`{:.path} - `파라미터 생성`{:.path}를 통해서 중요한 환경변수 값들을 생성합니다.
...
`> AWS CodeBuild로 Docker 이미지를 빌드하고 Docker Hub에 push`{:.img-caption}

## AWS CodeDeploy

---

AWS CodeDeploy는 애플리케이션 배포를 자동화하는 서비스입니다. 태그를 기반으로 대상 EC2의 codedeploy agent를 통해 `appspec.yml` 배포 스크립트를 수행합니다.
...
- BeforeInstall : cleanup.sh 스크립트로 기존의 Docker 컨테이너를 중지하고
- AfterInstall : update_container.sh 스크립트로 새로운 Docker 이미지를 pull 받아 컨터이너를 실행
...
AWS CodeDeploy가 지정된 태그를 가진 EC2에 codedeploy Agent로 아티팩트를 전송합니다. 아티팩트 내부에 `appspec.yml`을 통해 스크립트를 실행되어 배포가 이루어 집니다.
...
마지막으로 EC2 내부에 AWS CodeDeploy와 통신 할 수 있는 Agent를 설치합니다. [CodeDeploy 에이전트 설치 메뉴얼](https://docs.aws.amazon.com/ko_kr/codedeploy/latest/userguide/codedeploy-agent-operations-install-cli.html){:.none target="_blank"}을 참고해서 에이전트를 설치 합니다.
...
- AWS CodeBuild를 통해 Docker 이미지를 빌드 및 push
