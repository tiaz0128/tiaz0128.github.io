...
CA는 제출한 CSR을 검증하고, 이를 기반으로 인증서를 생성합니다. CA는 생성된 인증서에 자신의 비밀키로 디지털 서명을 합니다. 이를 통해 이후에 브라우저에서 이 인증서가 유효하다는 것을 CA의 공개키를 통해 검증 할 수 있게 됩니다.
...
인증서를 통해 서버의 공개키가 유효한 것은 알겠습니다. 그렇다면 인증서가 유효한지는 어떻게 알수 있을까요?
