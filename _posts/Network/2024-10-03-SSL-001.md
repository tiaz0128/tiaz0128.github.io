...
excerpt: "자체 서명된 인증서 → CA 인증서 발급 받아보기"
author: tiaz
permalink: /Network/1
tags: [SSL]
---
...
여기서 우리는 CA에서 인증서를 서버에 세팅는 과정을 해보겠습니다. 한번 해보고 다시 아래의 다이어그램을 보면 좀 더 쉽게 이해하실 수 있습니다.
...
CA 인증에 필요한 도메인을 `Public IPv4 DNS`를 사용 하겠습니다. VPC > VPC 설정 편집 > DNS 호스트 이름 활성화를 확인 합니다. 소유한 도메인이 있는 경우는 해당 도메인을 사용하면 됩니다.
...
우선은 자체 서명된 SSL 인증서를 만들어 보고, *NGINX*에 적용해보겠습니다.
...
- `req: X.509` : 인증서 서명 요청(CSR) 및 인증서를 생성하는 명령
- `-x509` : 공인 CA의 서명 없는 자체 서명된 인증서 생성
- `nodes` : "no DES"의 약자로, 개인키를 암호화하지 않음
- `days 365` : 인증서의 유효 기간을 365일(1년)로 설정
- `newkey rsa:2048` : 새로운 2048비트 RSA 키 쌍을 생성
- `keyout /etc/nginx/ssl/self.key` : 생성된 개인키를 저장할 경로를 지정
...
처음 SSL Certificates 에서 `Status`가 Incomplete 상태로 생성 됩니다. `View` 버튼을 누르고 `Generate Certificate` 버튼을 눌러서 서버 정보를 검증하고 인증서를 생성해 보겠습니다.
...
Certificate Signing Request(CSR)의 아래의 문장으로 시작하는 값을 이전 페이지에서 `Paste your CSR`에 붙여 넣어줍니다.
...
여기서 중요합니다! `Your Private Server Key`는 서버에 저장해야 하는 비밀 키입니다. 이 파일은 반드시 자신의 서버에 저장합니다. 아래의 경로에 해당 비밀 키를 저장하도록 하겠습니다.
...
Validation Methods에서 http를 선택합니다. 필요한 정보를 넣어줍니다.
...
SSL Certificates 에서 `Status`가 Processing 상태로 변경된 것을 확인 할수 있습니다. `Domain Validation` 탭에서 서버 인증용 파일(Validation File)을 받아서 서버에 저장 하겠습니다.
...
<div class="file-name">/usr/share/nginx/html/pki.txt</div>
...
`alias` 지시어를 사용하여 검증 URL이 해당 파일을 찾을 수 있게, 파일을 직접 지정해 줍니다. 이후 몇 분 기다려 보면 SSL Certificates 에서 `Status`가 Active 상태로 변경되면서 서버 인증이 끝났습니다.
...
server {
    listen 443 ssl;
    server_name ec2-3-36-45-12...;

    ...

    # CA가 인증 파일에 접근할 수 있도록 설정
    location = /.well-known/pki-validation/350105A2F3AE___.txt {
...
이 3개의 파일을 통해 인증서 체인(Chain of Trust)에 사용할 `fullchain.crt` 파일을 생성해야 합니다. 잠시 후에 만들어서 서버에 적용해 보도록 하겠습니다.

## CA 인증서 발급 정리

---

여기까지가 CA 인증서를 발급받는 과정입니다. 다이어그램의 순서를 다시 한번 보시면서 과정을 반복해보면 이해하기 쉽습니다.
...
`Online CSR Generator`를 사용하거나 이전 '자체 서명된 SSL 인증서' 발급하는 과정도 동일했습니다. 기억해보면 모두 크게 두개의 파일이 만들어졌습니다.

- 서버의 비밀 키
- CSR 파일

그리고 CSR 파일 내부에는 아래의 정보를 포함하게 됩니다.

- CSR 파일
  - 서버/조직 정보
  - 서버의 공개키
  - 서버의 비밀 키로 생성된 디지털 서명

### CA 인증서 생성

CA는 제출한 CSR을 검증하고, 이를 기반으로 SSL/TLS 인증서를 생성합니다. CA는 생성된 인증서에 자신의 개인키로 디지털 서명을 합니다. 이를 통해 이후에 브라우져에서 이 인증서가 유효하다는 것을 CA의 공개키를 통해 검증 할 수 있게 됩니다.
...
SSL 인증서를 기반으로 서버의 공개키가 유효한 것을 브라우저가 확인 했습니다. 검증한 서버의 공개키를 사용해 서버와 브라우저가 함께 사용할 대칭키를 안전하게 생성할 수 있게 됩니다. 이로써 https 통신이 가능하게 됩니다.
...
SSL은 인증서 체인을 통해 검증을 수행하도록 설계되어 있습니다. Fullchain은 이 계층 구조 속에서 최종 인증서부터 Root CA 직전까지의 모든 중간 단계를 포함하여, 클라이언트가 인증서의 유효성을 쉽고 안전하게 검증할 수 있도록 돕는 중요한 역할을 합니다.

![Fullchain](/assets/img/content/Network/001/014.png){:.img-m}

### 동작 방식

디지털 서명은 상위 CA의 비밀키로 인증서 내용(메타데이터, 소유자 정보, 공개키 등)을 해시한 후 암호화한 값입니다. 인증서 체인은 각 단계의 인증서가 상위 CA에 의해 서명되고, 이 서명을 해당 CA의 공개키로 검증하면서 신뢰할 수 있는 루트 CA까지 올라가는 구조를 가집니다. 이러한 구조로 인해 인증서들이 체인 형태로 연결되어 있다고 말합니다.

![Fullchain](/assets/img/content/Network/001/015.png){:.img-s}
...
NGINX에서 이 풀체인 파일을 사용할 때는 `ssl_certificate` 지시어에 이 파일을 지정하면 됩니다. 비밀 키 파일은 별도로 `ssl_certificate_key` 지시어에 지정 합니다.
...
server {
    ...
    ssl_certificate_key /etc/nginx/ssl/GoGetSSL_private.key; # 비밀 키
}
```
