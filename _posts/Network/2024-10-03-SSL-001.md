...
sequenceDiagram
    ...
    WS->>LCA: CSR 제출
    LCA->>LCA: 서버 인증서 생성 및 서명
    LCA->>ICA: 하위 CA 인증서 요청
    ICA->>ICA: 하위 CA 인증서 생성 및 서명
    ICA->>RCA: 중간 CA 인증서 요청
    RCA->>RCA: 중간 CA 인증서 생성 및 서명
    RCA-->>ICA: 서명된 중간 CA 인증서
    ICA-->>LCA: 서명된 하위 CA 인증서
    LCA-->>WS: 서명된 서버 인증서 + 인증서 체인
</pre>
